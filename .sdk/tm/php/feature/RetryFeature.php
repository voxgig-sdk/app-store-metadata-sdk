...
// AppStoreMetadata SDK retry feature
...
class AppStoreMetadataRetryFeature extends AppStoreMetadataBaseFeature
{
    ...
    public function init(AppStoreMetadataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AppStoreMetadataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AppStoreMetadataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AppStoreMetadataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

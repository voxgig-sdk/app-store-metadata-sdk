...
// AppStoreMetadata SDK feature test
//
...
class FtCtrl extends AppStoreMetadataControl
{
...
class FtClient
{
    ...
    private AppStoreMetadataUtility $utility;

    public function __construct(AppStoreMetadataUtility $utility, array $options)
    {
    ...
    public function get_utility(): AppStoreMetadataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AppStoreMetadataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AppStoreMetadataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AppStoreMetadataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AppStoreMetadataConfig::make_feature($name);
    ...
    private static function buildUrl(AppStoreMetadataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AppStoreMetadataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AppStoreMetadataResponse($fetched);
        }
    ...
    private function fail(AppStoreMetadataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AppStoreMetadataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

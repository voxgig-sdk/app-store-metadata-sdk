...
module AppStoreMetadataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AppStoreMetadataResponse.new({})
    ...
    if fetch_err
      ...
      response = AppStoreMetadataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AppStoreMetadataResponse.new(fetched)
    else

# app-store-metadata-cli

boru-driven command-line client **and** interactive REPL for the AppStoreMetadata
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/app-store-metadata-cli)
make build

# 2. See usage (words, entities, env vars)
./app-store-metadata-cli --help

# 3. Provide credentials once, via the environment
export APP_STORE_METADATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./app-store-metadata-cli load 1 app            # {id:1} shorthand
./app-store-metadata-cli load '{id:1}' app       # explicit match map

# 5. Override the API base URL for a single call
APP_STORE_METADATA_BASE=https://api.example.com ./app-store-metadata-cli load 1 app

# 6. No arguments -> interactive REPL
./app-store-metadata-cli
app-store-metadata> load 1 app
app-store-metadata> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/app-store-metadata-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/app-store-metadata-cli load 1 app
   ```
...
```sh
./app-store-metadata-cli load 1 app          # scalar shorthand for {id:1}
./app-store-metadata-cli load '{id:1}' app     # explicit match map
```
...
./app-store-metadata-cli load 1 app
```
...
Run with no arguments to open a REPL (prompt `app-store-metadata>`). Each line is
evaluated as its own boru expression:

```text
$ ./app-store-metadata-cli
app-store-metadata> load 1 app
app-store-metadata> /help
app-store-metadata> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `app`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/app-store-metadata-cli`. |
...
(`./app-store-metadata-cli <expr>`) and interactively (the REPL), and expressions compose the

// Command AppStoreMetadata-cli is a CLI + REPL for the AppStoreMetadata SDK,
...
const prompt = "app-store-metadata"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "app"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      APP_STORE_METADATA_APIKEY (api key), APP_STORE_METADATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: APP_STORE_METADATA_APIKEY carries the API key and
	// APP_STORE_METADATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("APP_STORE_METADATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("APP_STORE_METADATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAppStoreMetadataSDK(opts)

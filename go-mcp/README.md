...
[MCP](https://modelcontextprotocol.io) server exposing the AppStoreMetadata SDK as
two agent tools — `app-store-metadata_list` and `app-store-metadata_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/app-store-metadata-mcp)
make build

# 2. Provide credentials via the environment
export APP_STORE_METADATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user app-store-metadata \
  -- /absolute/path/to/app-store-metadata-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./app-store-metadata-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// app-store-metadata_list: first page of records
{ "entity": "app" }
{ "entity": "app", "query": { } }

// app-store-metadata_load: one record by id
{ "entity": "app", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/app-store-metadata-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user app-store-metadata \
     -- "$PWD"/dist/*/app-store-metadata-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `app-store-metadata_list` and `app-store-metadata_load` tools now appear
   in new sessions. Ask the agent to *"list app using app-store-metadata"*
   and it calls `app-store-metadata_list` with `{"entity":"app"}`.
...
```sh
export APP_STORE_METADATA_APIKEY=sk_live_xxx            # API key
export APP_STORE_METADATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `app-store-metadata_list` tool
...
### Call the `app-store-metadata_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `app-store-metadata_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `app-store-metadata_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `APP_STORE_METADATA_APIKEY` | API key sent with every request. |
| `APP_STORE_METADATA_BASE` | Optional override of the API base URL. |
...
```sh
./app-store-metadata-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"app-store-metadata_load","arguments":{"entity":"app","query":{"id":1}}}}'
```

...
import (
	...
	omni "github.com/voxgig-sdk/app-store-metadata-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AppStoreMetadataError); is {
				out := map[string]any{
					"name":    "AppStoreMetadataError",
...
func omniCtx(arg any, client *sdk.AppStoreMetadataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AppStoreMetadataSDK); is {
			client = live

...
require("app-store-metadata_types")
...
local AppStoreMetadataSDK = {}
AppStoreMetadataSDK.__index = AppStoreMetadataSDK
...
AppStoreMetadataSDK._make_feature = _make_feature


function AppStoreMetadataSDK.new(options)
  local self = setmetatable({}, AppStoreMetadataSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function AppStoreMetadataSDK:options_map()
...
function AppStoreMetadataSDK:get_utility()
  return Utility.copy(self._utility)
end


function AppStoreMetadataSDK:get_root_ctx()
  return self._rootctx
end


function AppStoreMetadataSDK:prepare(fetchargs)
...
function AppStoreMetadataSDK:direct(fetchargs)
...
function AppStoreMetadataSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function AppStoreMetadataSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "AppStoreMetadataSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function AppStoreMetadataSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function AppStoreMetadataSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "AppStoreMetadataSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function AppStoreMetadataSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = AppStoreMetadataSDK.new(sdkopts)
  sdk.mode = "test"

...
local AppEntity = {}
AppEntity.__index = AppEntity


function AppEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AppEntity)
  self._name = "app"
  self._client = client
...
function AppEntity:get_name()
  return self._name
end


function AppEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AppEntity.new(self._client, opts)
end
...
function AppEntity:mark_deleted()
  self._deleted = true
end


function AppEntity:deleted()
  return true == self._deleted
end


function AppEntity:data_set(args)
  if args ~= nil then
...
function AppEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AppEntity:match_set(args)
  if args ~= nil then
...
function AppEntity:stream(action, args, callopts)
...
---@param reqmatch AppLoadMatch
---@param ctrl? table
---@return App
---@return string? err
function AppEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function AppEntity:_run_op(ctx, post_done)
  local utility = self._utility

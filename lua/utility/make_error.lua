...
local function make_error_util(ctx, err)
  ...
  local msg = "AppStoreMetadataSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = AppStoreMetadataError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == AppStoreMetadataError then

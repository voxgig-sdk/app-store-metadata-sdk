# AppStoreMetadata SDK test runner
...
class AppStoreMetadataTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AppStoreMetadataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AppStoreMetadataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AppStoreMetadataTestRunner.getenv("APP_STORE_METADATA_TEST_LIVE")
        override = AppStoreMetadataTestRunner.getenv("APP_STORE_METADATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AppStoreMetadataTestRunner.getenv(key)
        ...
        explain = AppStoreMetadataTestRunner.getenv("APP_STORE_METADATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if AppStoreMetadataTestRunner._test_control is not None:
            return AppStoreMetadataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AppStoreMetadataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AppStoreMetadataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AppStoreMetadataTestRunner.load_test_control()
...
def load_env_local():
    AppStoreMetadataTestRunner.load_env_local()


def env_override(m):
    return AppStoreMetadataTestRunner.env_override(m)


def entity_data(v):
    return AppStoreMetadataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AppStoreMetadataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AppStoreMetadataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AppStoreMetadataTestRunner.load_test_control()


def live_client_options():
    return AppStoreMetadataTestRunner.live_client_options()


def live_delay_ms():
    return AppStoreMetadataTestRunner.live_delay_ms()

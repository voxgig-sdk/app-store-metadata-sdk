...
require_relative 'AppStoreMetadata_types'
...
class AppStoreMetadataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AppStoreMetadataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AppStoreMetadataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AppStoreMetadataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AppStoreMetadataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AppStoreMetadataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AppStoreMetadataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AppStoreMetadataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => AppStoreMetadataError.new(
        "#{op}_allow",
        "AppStoreMetadataSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue AppStoreMetadataError => err
    ...
    if fetched.is_a?(Hash)
      status = AppStoreMetadataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AppStoreMetadataError.new(
        "graphql_error", "AppStoreMetadataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AppStoreMetadataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end

...
class AppEntity
  ...
  def make
    opts = @_entopts.dup
    AppEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AppStoreMetadataHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = AppStoreMetadataHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.App.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AppStoreMetadataHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

...
//     (\`new AppStoreMetadataSDK(...)\` / \`AppStoreMetadataSDK.test(...)\` become
//     \`AppStoreMetadataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AppStoreMetadataSDK'
...
const TEST_SEED = {"entity":{"app":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AppStoreMetadataSDK(...)\` and any
// \`AppStoreMetadataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AppStoreMetadataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AppStoreMetadataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
